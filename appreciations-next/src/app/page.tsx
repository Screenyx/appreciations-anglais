...
function Card({ item, profilLabel, searchText, onCopy, onEdit, onDelete, onRestore }:
  ...
  return (
    <div onClick={copy} className={`group relative rounded-lg p-3 cursor-pointer transition-all border-[1.5px] flex flex-col gap-2 ${copied ? 'border-green-400' : 'border-[var(--border)] hover:border-[var(--ink)] hover:shadow-md hover:-translate-y-px'} ${isCustom ? 'border-l-[3px] !border-l-[var(--accent)]' : ''} ${isEdited ? 'border-l-[3px] !border-l-[var(--accent3)]' : ''}`}
      style={{ background: copied ? '#f0f7f3' : 'var(--card)' }}>
      <div className="absolute top-2 right-2 hidden group-hover:flex gap-1 z-10">
        <button onClick={e => { e.stopPropagation(); onEdit() }} className="p-1 rounded hover:bg-[var(--tag-bg)]" style={{ color: 'var(--muted)' }}><EditIcon /></button>
        <button onClick={e => { e.stopPropagation(); onDelete() }} className="p-1 rounded hover:bg-red-50" style={{ color: 'var(--muted)' }}><TrashIcon /></button>
        {onRestore && <button onClick={e => { e.stopPropagation(); onRestore() }} className="p-1 rounded text-xs font-bold hover:bg-blue-50" style={{ color: 'var(--accent3)' }}>↺</button>}
      </div>
      <p className="text-[13px] leading-relaxed flex-1 pr-12" style={{ color: 'var(--ink)' }}>{hl(item.texte)}</p>
      <div className="flex items-end justify-between gap-2">
        ...
        <span className="text-[10px] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap" style={{ color: copied ? 'var(--accent2)' : 'var(--muted)' }}>
          {copied ? '✓ Copié !' : 'Copier'}
        </span>
      </div>
    </div>
              ...
              return (
                <div key={key} className="mb-8">
                  <div className="flex items-center gap-2 mb-3 pb-2 border-b-2" style={{ borderColor:'var(--border)' }}>
                    ...
                    <span className="ml-auto text-xs px-2 py-0.5 rounded" style={{ background:'var(--tag-bg)',color:'var(--muted)' }}>{items.length}</span>
                  </div>
                    ...
                    {Object.entries(td.profils).map(([profil,texts])=>(
                      <div key={profil}>
                        ...
                        {(texts as string[]).map((texte,i)=>{
                          ...
                          return (
                            <div key={i} className={`group relative rounded-lg px-3 py-2.5 cursor-pointer text-xs leading-relaxed border-[1.5px] transition-all hover:border-[var(--ink)] ${isEdited?'border-l-2':'border-transparent'}`}
                              ...
                              <div className="absolute top-1.5 right-2 hidden group-hover:flex gap-1">
                                <button onClick={e=>{e.stopPropagation();setModal({mode:'bilan',editBilan:{texte:display,classe:c,trim:t as Trimestre,profil,id:'builtin||'+key}})}}
                                  className="p-1 rounded" style={{ color:'var(--muted)' }}><EditIcon /></button>
                                <button onClick={e=>{e.stopPropagation();if(confirm('Masquer ?')){store.overrideBuiltinBilan(key,'deleted');showToast('Masqué.')}}}
                                  className="p-1 rounded" style={{ color:'var(--muted)' }}><TrashIcon /></button>
                                {isEdited&&<button onClick={e=>{e.stopPropagation();store.restoreBuiltinBilan(key);showToast('Restauré.')}} className="p-1 rounded text-xs font-bold" style={{ color:'var(--accent3)' }}>↺</button>}
                              </div>
                            </div>
                          )
                        })}
                        {store.bilans.filter(b=>b.classe===c&&b.trim===t&&b.profil===profil).map(b=>(
                          <div key={b.id} className="group relative rounded-lg px-3 py-2.5 cursor-pointer text-xs leading-relaxed border-[1.5px] border-l-2 transition-all hover:border-[var(--ink)]"
